...
has 'monitor'	=> (
	is 		=>	'rw',
	isa 	=> 'Agua::Monitor::SGE',
	default	=>	sub { Agua::Monitor::SGE->new({});	}
);
...
method setStages ($username, $cluster, $json, $project, $workflow) {
	...
	#### GET MONITOR
	$self->logDebug("BEFORE monitor = self->setQueueMonitor()");
	my $monitor = $self->setQueueMonitor();
	$self->logDebug("AFTER monitor = self->setQueueMonitor()");
...
method createQueue ($username, $cluster, $project, $workflow, $envars) {
	...
	my $adminuser = $self->conf()->getKey("agua", "ADMINUSER");
	my $configfile;
	$configfile =  $self->getConfigFile($username, $cluster) if not $adminkey;
	$configfile =  $self->getConfigFile($username, $cluster, $adminuser) if $adminkey;
...
method setQueue ($queue, $qmasterport, $execdport) {
	$self->logDebug("DOING slots = self->setSlots(self->instancetype())");
...
ORDER BY number
	...
	return $self->_getLocalStatus($username, $project, $workflow, $stages) if not $cluster;

	#### OTHERWISE, GET CLUSTER STATUS
	return $self->_getClusterStatus($username, $cluster, $project, $workflow, $stages);
}

method _getClusterStatus($username, $cluster, $project, $workflow, $stages) {
	$self->logDebug("");	
	...
	my $clusterstatus = $self->_emptyClusterStatus();
	$clusterstatus->{cluster}	=	$cluster;
	$clusterstatus->{status}	=	"none";
	...
	##### SKIP IF WORKFLOW IS PENDING
	#return $status if $workflowstatus eq "pending";
	...
	return $status if $currentstatus eq "cluster pending";
	...
	#### IF CLUSTER IS RUNNING GET QUEUE STATUS
	if ( $clusterrunning ) {
...
method _getLocalStatus ($username, $project, $workflow, $stages) {
#### PRINT stages AND RETURN IF CLUSTER IS NOT DEFINED
