...
class Xy::Configure with (Xy::Common::Util, Xy::Common::Logger) {
...
has 'db'	=> ( isa => 'Xy::DBase::MySQL', is => 'rw', required => 0 );
has 'conf' 	=> (
	is =>	'rw',
	'isa' => 'Conf::Yaml',
...
sub config {
	...
	#### CREATE MYSQL USERS: 'root', Agua user and Agua Test user
	$self->mysql();
	
	#### RECREATE TABLES AND LOAD DATA (OPTIONAL: BACKUP EXISTING DATABASE)
	$self->loadDatabase();
...
This configuration utility does the following:
	...
	3. Set the application's MySQL user name and password

	4. Set the test MySQL user name and password
...
sub setDbObject {
   ...
   #### CREATE DB OBJECT USING DBASE FACTORY
    my $db = Xy::DBaseFactory->new( 'MySQL',
        {
			database	=>	$database,
            user      	=>  $user,
            password  	=>  $password,
...
#### PROMPT TO SET ROOT MYSQL PASSWORD
	...
	print "\n";
	my $resetroot = qq{Reset MySQL root password (Y/N)?};
...
sub loadDatabase {
	...
	#### CHECK FOR QUIT
    print "\nDo you want to drop the existing tables and reload the database?\n(You will asked if you want to save the existing database)\n\n";
	exit if not $self->yes("Type 'Y' to continue or 'N' to cancel");
