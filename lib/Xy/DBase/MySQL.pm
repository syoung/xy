...
class Xy::DBase::MySQL extends Xy::DBase with Xy::Common::Logger {
...
#### LOAD DATA FROM .TSV FILE INTO TABLE
	...
	if ( not $count ) {   print "Xy::DBase::MySQL    DID NOT LOAD!\n";    }
	$self->dbh()->{RaiseError} = 1; # ENABLE TERMINATION ON ERROR
}
...
print "Xy::DBase::MySQL::useDatabase    database: $database\n";
...
#### RETURN 1 IF A DATABASE EXISTS WITH THE GIVEN NAME, OTHERWISE RETURN 0
	my $user = $self->user();
	my $password = $self->password();
	die "Xy::DBase::MySQL::isDatabase    user not defined\n" if not defined $user;
	die "Xy::DBase::MySQL::isDatabase    password not defined\n" if not defined $password;
