...
method testTransferManifest {
	diag("# Test transferManifestFile");
	
	my $tests = [
		{
			testname	=> 	"transfer manifest",
			sourcefile	=> 	"$Bin/inputs/csvfiles/manifest-III_Test3.csv",
			params		=>	{
				tempfile	=> 	"$Bin/outputs/files/project/manifest-III_Test3.csv",
				username	=>	"guest",
				mode		=>	"manifest",
				project_name=>	"III_Test3",
				filename	=> 	"manifest-III_Test3.csv",
			}
		}
	];
	...
	foreach my $test ( @$tests ) {
		...
		#### SET INSTALLDIR
		my $entry = $self->conf()->getKey("agua", undef);
		$entry->{INSTALLDIR} = "$Bin/outputs";
		$self->conf()->setKey("agua", $entry);
		
		#### COPY INPUT FILE TO outputs
		my $sourcefile	=	$test->{sourcefile};
		my $tempfile	=	$test->{params}->{tempfile};	
		$self->setUpFile($sourcefile, $tempfile);
		
		my $success = $self->transferManifest($test->{params});		
		ok($success, $test->{testname});
	}	
}

method testParseManifest {
	diag("# Test parseManifest");
	...
	use Conf::Yaml;
	my $conf = new Conf::Yaml({
		inputfile	=>	"$installdir/conf/config.yaml"
	});
	my $database    =   $conf->getKey("database", "TESTDATABASE");
	my $testuser    =   $conf->getKey("database", "TESTUSER");
	my $testpassword=   $conf->getKey("database", "TESTPASSWORD");
	...
	#### SET CONF DATABASE ENTRIES
	$entry = $conf->getKey("database", undef);
	$entry->{DATABASE} 		=	$database;
	$entry->{TESTDATABASE} 	=	$database;
	$entry->{TESTUSER} 		=	$testuser;
	$entry->{TESTPASSWORD} 	=	$testpassword;	
	$self->conf()->setKey("database", $entry);

	##### CREATE DATABASE WITHOUT DATA
	...
	my $tests = [
		{
			...
			params		=>	{
				tempfile	=> 	"$Bin/inputs/csvfiles/manifest-III_Test3.csv",
				username	=>	"guest",
				mode		=>	"manifest",
				project_name=>	"III_Test3",
				filename	=> 	"manifest-III_Test3.csv",
		...
		,
		{
			testname	=> 	"project exists already",
			success		=> 	1,
			expectedfile=>	"$Bin/inputs/json/project-exists.json",
			params		=>	{
				tempfile	=> 	"$Bin/inputs/csvfiles/manifest-III_Test4.csv",
				username	=>	"guest",
				mode		=>	"manifest",
		...
		,
		{
			testname	=> 	"cannot parse project name",
			success		=> 	0,
			expectedfile=>	"$Bin/inputs/json/project-badname.json",
			params		=>	{
				tempfile	=> 	"$Bin/inputs/csvfiles/manifest-III_Test5.csv",
				username	=>	"guest",
				mode		=>	"manifest",
				project_name=>	"III_Test5",
				filename	=> 	"manifest-III_Test5.csv",
				error		=>	"Cannot parse project name from manifest file. Verify that all five lines are present : Institute Name, Principal Investigator, Project Name, Date Received, Description"
			}
		}
		,
		{
			testname	=> 	"bad sample line",
			success		=> 	1,
			expectedfile=>	"$Bin/inputs/json/project-badsample.json",
			params		=>	{
				tempfile	=> 	"$Bin/inputs/csvfiles/manifest-III_Test6.csv",
				username	=>	"guest",
				mode		=>	"manifest",
				project_name=>	"III_Test6",
				filename	=> 	"manifest-III_Test6.csv",
				error		=>	"Failed to validate field 'gender' (value: X) in line 5 of samples"
			}
		}
	];
	...
	foreach my $test ( @$tests ) {
		...
		#### LOAD TSVFILE
		my $tsvfile = "$Bin/inputs/tsvfiles/project.tsv";
		$self->loadTsvFile("project", $tsvfile);

		my ($params, $error) = $self->parseManifest($test->{params});		
		$self->logDebug("params", $params);
		#$self->logDebug("error", $error);
		$self->logDebug("test->params->error", $test->{params}->{error});
		
		#### TEST ERROR
		is_deeply($error, $test->{params}->{error}, "error - $test->{testname}");
		...
		my $success = 0;
		$success = 1 if defined $params->{project_name};
		is_deeply($success, $test->{success}, "project_name - $test->{testname}");
