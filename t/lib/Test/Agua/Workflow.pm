...
class Test::Agua::Workflow with (Test::Agua::Common::Database,
...
method initialise ($hash) {
    ...
    #### SET STARCLUSTER SHOWLOG/PRINTLOG    
    $self->starcluster()->SHOWLOG($self->SHOWLOG());
    $self->starcluster()->PRINTLOG($self->PRINTLOG());
}
...
#### RETURN INSTANCE OF StarCluster
    ...
    my $clusterobject = {
        ...
        configfile      =>  "$Bin/outputs/loadstarcluster/aguatest/.starcluster/aguatest-testcluster.config",
        outputdir       =>  "$Bin/outputs/loadstarcluster/aguatest/.starcluster"
    };
	...
	foreach my $slot ( keys %$clusterobject ) {
		ok($self->starcluster()->$slot() ne $clusterobject->{$slot}, "$slot value is NOT $clusterobject->{$slot}") if $self->starcluster()->can($slot) and defined $self->starcluster()->$slot();
	}
	...
	foreach my $slot ( keys %$clusterobject ) {
       ...
       $self->logDebug("self->starcluster()->$slot()", $self->starcluster()->$slot()) if $self->starcluster()->can($slot);
 		ok($self->starcluster()->$slot() eq $clusterobject->{$slot}, "$slot value IS $clusterobject->{$slot}") if $self->starcluster()->can($slot) and defined $self->starcluster()->$slot();
	}
...
method setUpStarCluster ($testname, $username, $cluster) {
    ...
    $self->conf()->getKey("agua", 'INSTALLDIR', "$Bin/outputs/$testname");
...
method testExecuteWorkflow {
	...
	*{updateClusterStatus} = sub {
	...
	$self->starcluster()->username("aguatest");
	$self->starcluster()->cluster("aguatest-testcluster");
...
method testGetStatus {
	...
	$expected = $self->copyNow($status, $expected);
...
#### UTILS
method copyNow ($source, $target) {
	for ( my $i = 0; $i < @{$source->{stagestatus}}; $i++ ) {
		${$target->{stagestatus}}[$i]->{now} = ${$source->{stagestatus}}[$i]->{now};
	}
